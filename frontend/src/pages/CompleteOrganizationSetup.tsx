...
const CompleteOrganizationSetup = () => {
  ...
  useEffect(() => {
    // Load pending organization setup data if available
    const pendingSetup = localStorage.getItem("pending_org_setup");
    if (pendingSetup) {
      try {
        ...
        setSelectedPlan(data.selected_plan || "PRO");
      } catch (err) {
        console.error("Error loading pending setup:", err);
      }
    }

    // Verify user is authenticated and eligible
    if (!user) {
      navigate("/login");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.AUTH.ORGANIZATION_SETUP, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          max_storage_gb: formData.max_storage_gb
            ? parseInt(formData.max_storage_gb)
            : null,
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.detail || "Organization setup failed");
      }
      ...
      // Redirect to pending approval page after showing success message
      setTimeout(() => {
          ...
          <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <div className="text-center">
                ...
                >
                  View Status <ArrowRight size={20} />
                </button>
              </div>
            </div>
          </div>
...
export default CompleteOrganizationSetup;
